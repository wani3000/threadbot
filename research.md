...
- 현재 구조에서 실제 첫 게시/연속 스레드 분할 기준은 빈 줄(`\n\n`) 또는 `---` 구분자다
- 따라서 생성 결과가 짧은 문단 여러 개로 나오면 너무 짧은 연속 스레드가 다수 생길 수 있다
...
## 13. 운영 규칙 변경: 스레드 단위 최소 150자

### 요구사항
- 첫 게시와 연속 스레드 각각이 최소 150자 이상이어야 한다
- 너무 짧은 문단이 독립 포스트로 올라가면 안 된다

### 적용한 수정
- `vercel/lib/generate.ts`
  - 생성 검증 단계에서 각 문단이 최소 150자 미만이면 재시도 대상으로 본다
  - 시스템 프롬프트와 재시도 프롬프트에 `첫 게시/각 연속 스레드 최소 150자` 규칙을 추가했다
- `vercel/lib/threads.ts`
  - 실제 게시 직전 분할 결과를 확인하고, 150자 미만 문단은 앞뒤 문단과 병합해 게시 단위를 재구성한다
  - 분할 후 남는 짧은 마지막 조각은 직전 게시 단위에 합쳐서 단독 업로드를 막는다

### 기대 효과
- 지나치게 짧은 한두 줄짜리 연속 스레드 방지
- 실제 게시 결과가 읽기 흐름상 덜 파편화됨
...
- 2026-03-09: 첫 게시/연속 스레드 최소 150자 규칙 반영 완료.
